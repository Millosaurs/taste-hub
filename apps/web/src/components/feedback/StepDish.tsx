...
export function StepDish() {
  ...
  return (
    <div className="min-h-svh bg-base pb-24 sm:pb-28">
      <div className="max-w-2xl mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
        {/* Header */}
        <div className="text-center space-y-1 sm:space-y-2">
          ...
          {selectedDishes.length > 0 && (
            <p className="text-accent-yellow text-xs sm:text-sm font-medium">
          ...
          {dishes.map((dish) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative overflow-hidden rounded-xl sm:rounded-[14px] border-2 transition-all duration-75 text-left",
              ...
              >
                {/* Image */}
                <div className="aspect-square sm:aspect-[4/3] relative bg-raised">
                  {dish.image_url ? (
                    <Image
                      src={dish.image_url}
                      alt={dish.name}
                  ...
                  {/* Selection indicator */}
                  {selected && (
                    <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 w-5 h-5 sm:w-6 sm:h-6 bg-accent-yellow rounded-full flex items-center justify-center shadow-sm">
                      <span className="text-xs sm:text-sm text-white font-bold">✓</span>
                    </div>
                  )}
                </div>
                {/* Name */}
                <div className="p-1.5 sm:p-3 bg-surface">
                  <span className="text-[11px] sm:text-sm font-medium text-text-primary line-clamp-2 leading-tight">
                    {dish.name}
                  </span>
                </div>
              </button>
